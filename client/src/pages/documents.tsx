  ...
  return (
    <>
      ...
      <div className="aura-topbar">
        <span className="aura-topbar-title">⊟ {t("nav.documents")}</span>
        <button className="a-btn a-btn-primary" onClick={() => fileRef.current?.click()} data-testid="button-upload-document">
      ...
      <div className="aura-page">
        <div className="a-page-hd a-anim">
          <div>
            <div className="a-page-title">{t("documents.title")}</div>
            <div className="a-page-sub">{t("documents.subtitle")}</div>
          </div>
          <div style={{ display: "flex", gap: "0.5rem" }}>
            <div className="kb-stat"><span style={{ color: "#60a5fa", fontWeight: 800 }}>{docs.length}</span><span>{t("documents.uploadedDocuments")}</span></div>
            <div className="kb-stat"><span style={{ color: "#34d399", fontWeight: 800 }}>{docs.filter(d => d.status === "processed").reduce((a, d) => a + d.chunks, 0)}</span><span>{t("documents.chunks")}</span></div>
          </div>
        </div>
        ...
        >
          ...
          <div className="drop-sub">{t("documents.dropZoneSub")}</div>
        ...
        <div className="a-card a-anim" style={{ animationDelay: "0.1s", padding: 0, overflow: "hidden" }}>
          <div style={{ padding: "1rem 1.4rem 0.75rem", borderBottom: "1px solid rgba(245,158,11,0.08)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ fontSize: "0.88rem", fontWeight: 700, color: "#c8a96e" }}>{t("documents.uploadedDocuments")}</span>
            <span style={{ fontSize: "0.7rem", color: "#6b6355" }}>{docs.length} {docs.length !== 1 ? t("documents.files") : t("documents.file")}</span>
          </div>
          ...
          {docs.length === 0 ? (
            ...
            <div>
              {docs.map((doc, i) => (
                <div key={doc.id} className="doc-row" style={{ animationDelay: `${i * 0.04}s` }} data-testid={`document-${doc.id}`}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "0.6rem", marginLeft: "auto" }}>
                    ...
                    <button className="doc-action-btn" title={t("documents.reprocess")} data-testid={`button-reprocess-${doc.id}`}>↺</button>
                    <button className="doc-action-btn red" onClick={() => deleteDoc(doc.id)} title={t("common.delete")} data-testid={`button-delete-${doc.id}`}>✕</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="a-card a-anim" style={{ animationDelay: "0.15s", background: "rgba(245,158,11,0.04)", border: "1px solid rgba(245,158,11,0.12)" }}>
          <div style={{ fontSize: "0.8rem", fontWeight: 700, color: "#c8a96e", marginBottom: "0.6rem" }}>{t("documents.tipsTitle")}</div>
...
const css = `
  ...
  .kb-stat{display:flex;flex-direction:column;align-items:center;background:#141109;border:1px solid rgba(245,158,11,.1);border-radius:10px;padding:.5rem .9rem;font-size:.68rem;color:#6b6355;gap:.1rem;min-width:72px;}
