  ...
  return (
    <>
      ...
      <div className="aura-topbar">
        <span className="aura-topbar-title">◇ Conversations</span>
        <div className="conv-filter-tabs">
          {["all","open","resolved"].map(f => (
            <button key={f} className={`filter-tab ${filter === f ? "active" : ""}`} onClick={() => setFilter(f)}>
              {f.charAt(0).toUpperCase() + f.slice(1)}
              {f === "open" && <span className="filter-dot" />}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="conv-layout">
        ...
        <div className="conv-list">
          {filtered.map((t, i) => (
            <button
              key={t.id}
              className={`conv-item ${selected?.id === t.id ? "active" : ""}`}
              onClick={() => setSelected(t)}
              style={{ animationDelay: `${i * 0.05}s` }}
            >
              <div className="conv-av">{t.name.charAt(0)}</div>
              <div className="conv-item-info">
                <div className="conv-item-top">
                  <span className="conv-item-name">{t.name}</span>
                  <span className="conv-item-time">{t.time}</span>
                </div>
                <div className="conv-item-preview">{t.preview}</div>
              </div>
              <div className={`conv-status-dot ${t.resolved ? "resolved" : "open"}`} />
            </button>
          ))}
        </div>
        ...
        {selected ? (
          <div className="conv-thread">
            <div className="thread-header">
              ...
              <div>
                ...
                <div className="thread-meta">{selected.msgs} messages · {selected.time}</div>
              </div>
            ...
            <div className="thread-footer">
              <div style={{ fontSize: "0.72rem", color: "#4a4035", display: "flex", alignItems: "center", gap: "0.4rem" }}>
                <span>◈</span> This conversation was handled by your AI agent
              </div>
              <div style={{ display: "flex", gap: "0.5rem" }}>
                <button className="a-btn a-btn-ghost-sm">Export</button>
                <button className="a-btn a-btn-ghost-sm">⚑ Flag</button>
              </div>
            </div>
          </div>
        ) : (
          <div className="conv-empty">
            ...
            <div style={{ fontSize: "0.85rem", color: "#6b6355" }}>Select a conversation</div>
          </div>
        )}
      </div>
    </>
